...
class SharedPsumPool:
    ...
    def try_flush_to_accumulator(self):
        ...
        for buf in self.buffers:
            ...
            self.accumulator.request_bundle(
                core_id=self.core_id,
                rows=[buf["target_r"]],
                cols=cols,
                data=[buf["data"][col].clone() for col in cols],
                kind=kind,
                # 仲裁完成后再清除已写回列；失败的请求保留到下一拍重试。
                on_result=lambda success, buf=buf, cols=cols, is_stream=is_stream, kind=kind:
                    self._finish_conv_retire(buf, cols, is_stream, kind, success),
            )

    def _finish_conv_retire(self, buf, cols, is_stream, kind, success):
    ...
    def try_retire_fc_bundle(self, row_buffers, col, on_result=None):
        ...
        def complete(accepted_mask):
            success = self._handle_fc_partial_result(bundle, accepted_mask, is_pending=False)
            if on_result is not None:
                on_result(success)

        self.accumulator.request_bundle_partial(
            core_id=self.core_id,
            rows=bundle["rows"],
            cols=bundle["cols"],
            data=bundle["data"],
        ...
        # No synchronous acceptance: all cores must present their requests.
        return None
    ...
    def _make_fc_bundle(self, row_buffers, col):
        ...
        ordered = [
            ...
            # Rejected data already belongs to pending_fc_bundles; Core must
            # not resubmit its cleared source columns on a subsequent retry.
            if bool(buf['modified'][col].item())
        ]
        if not ordered:
            return None
    ...
    def _try_flush_pending_fc_bundle(self):
        ...
        self.accumulator.request_bundle_partial(
            ...
            cols=bundle["cols"],
            data=bundle["data"],
            on_result=lambda mask: self._handle_fc_partial_result(bundle, mask, is_pending=True),
        )
        return None
