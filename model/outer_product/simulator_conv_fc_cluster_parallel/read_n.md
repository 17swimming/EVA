# 性能统计
由于有多个core，我觉得统计所有core的平均值才有意义，比如compute cycle=10，如果6个core中累积前端stall了12次（某个cycle，若6个core有3个core在等待package，前端stall应该+3），那前端stall的比例应该按照12/（10x6）= 20% ，这样统计才有意义。

每拍对每个 Core 依次判断：
- 成功处理 package：正常计算。
- Split 尚未结束、取不到 package：前端停顿。
- Psum 分配失败：Psum 分配停顿。
- Split 已结束、没有待处理 package，但 Psum pool 仍有待写回数据：写回排空。
- 以上均不满足：空闲/调度。
因此，最后一类包含：
1. Core 提前完成，当前 Cin 队列已空，等待其他 Core。
2. 完成状态确认等控制周期。
3. 所有 Core 已完成，但 accumulator 尚未清空的尾部周期。 当前这些周期也归入空闲，而不是写回排空。
